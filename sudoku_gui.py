...
def valid(sq, num, pos):
    # Check row
    for i in range(len(sq[0])):
        if sq[pos[0]][i] == num and pos[1] != i:
            return False

    # Check column
    for i in range(len(sq)):
        if sq[i][pos[1]] == num and pos[0] != i:
            return False
...
def mainRun():
    ...
    while run:
        ...
        redrawWindow(win, board, play, strikes)
        pygame.display.update()
